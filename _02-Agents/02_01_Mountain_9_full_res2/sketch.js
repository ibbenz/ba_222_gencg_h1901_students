...
function setup() {
  ...
  mountains= new Array(options.numberOfMountains);
    for (let j = 0; j <= options.numberOfMountains; j++) {
        ...
        mountains[j]=new mountainElement(options.numberOfPeaks,(windowHeight1 / options.numberOfMountains) * j,1.5*windowWidth1,1);
        temporaryWave[j]=0;
...
function draw() {
    ...
    if (start == false) {
        ...
        for (let j = 0; j <= options.numberOfMountains; j++) {
            ...
            mountains[j] = new mountainElement(options.numberOfPeaks, (windowHeight1 / (options.numberOfMountains+1)) * j, 1.5*windowWidth1, 1);
            temporaryWave[j] = 0;
            temporaryWaveCounter[j] = 0;
        }
